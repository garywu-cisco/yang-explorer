...
class Command(BaseCommand):
    ...
    def _reload_cxml(self, username):
        ...
        user.userprofile_set.delete()

        # create new UserProfiles from CXML files
        for f in os.listdir(os.path.join(django.conf.settings.BASE_DIR,
                                         'data', 'users', username, 'cxml')):
            f_split = os.path.splitext(f)
            if f_split[1] == '.xml':
                profile = UserProfile(user=guest, module=f_split[0])
                profile.save()
